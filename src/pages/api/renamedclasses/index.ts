...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createRenamedclass() {
    ...
    const body = { ...req.body };
    if (body?.class_registration?.length > 0) {
      const create_class_registration = body.class_registration;
      body.class_registration = {
        create: create_class_registration,
      };
    } else {
      delete body.class_registration;
    }
